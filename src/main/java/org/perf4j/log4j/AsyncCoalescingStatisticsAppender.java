...
/**
 * This log4j Appender groups StopWatch log messages together to form
 * GroupedTimingStatistics. At a scheduled interval
 * the StopWatch log messages that currently exist in the buffer are pulled to
 * create a single
 * GroupedTimingStatistics instance that is then sent to any attached appenders.
 * <p/>
 * Note that any LoggingEvents which do NOT contain StopWatch objects are
 * discarded. Also, this appender stores logged
 * messages in a bounded buffer before sending those messages to downstream
 * appenders. If the buffer becomes full then
 * subsequent logs will be discarded until the buffer has time to clear. You can
 * access the number of discarded
...
public class AsyncCoalescingStatisticsAppender extends AppenderSkeleton implements AppenderAttachable {
    ...
    // note most configuration options are provided by the
    ...
    // --- contained objects ---
    /**
     * This instance provides the main logic for this appender. This wrapper class
     * just provides the log4j-specific
     * parts.
     */
    ...
    /**
     * This shutdown hook is needed to flush the appender on JVM shutdown so that
    ...
    // --- options ---
    /**
     * The <b>TimeSlice</b> option represents the length of time, in milliseconds,
     * of the window in which appended
     * LoggingEvents are coalesced to a single GroupedTimingStatistics and sent to
    ...
    /**
     * The <b>DownstreamLogLevel</b> option gets the Level of the
     * GroupedTimingStatistics LoggingEvent that is sent to
     * downstream appenders. Since each GroupedTimingStatistics represents a view of
     * a collection of single StopWatch
     * timing event, each of which may have been logged at different levels, this
     * appender needs to decide on a single
    ...
    /**
     * Sets the value of the <b>DownstreamLogLevel</b> option. This String must be
     * one of the defined Level constants.
     *
    ...
    /**
     * The <b>CreateRollupStatistics</b> option is used to determine whether
     * "rollup" statistics should be created.
     * If the tag name of a StopWatch in a log message contains periods, then the
     * GroupedTimingStatistics will be
     * created as if each substring of the tag up to the period was also logged with
     * a separate StopWatch instance.
     * For example, suppose a StopWatch was logged with a tag of
     * "requests.specificReq.PASS". For grouping purposes
     ...
     * </ul>
     * This allows you to view statistics at both an individual and aggregated
     * level. If there were other StopWatch
     * entries with a tag of "requests.specificReq.FAIL", then the data collected at
     * the "requests.specificReq" level
    ...
    /**
     * The <b>QueueSize</b> option is used to control the size of the internal queue
     * used by this appender to store
     * logged messages before they are sent to downstream appenders. Defaults to
     * 1024. If set too small and the queue
     * fills up, then logged StopWatches will be discarded. The number of discarded
     * messages can be accessed using the
    ...
    /**
     * The <b>StopWatchParserClassName</b> option is used to determine the class
     * used to parse stop watch messages
     * into StopWatch instances. This defaults to the standard
     * "org.perf4j.helpers.StopWatchParser" class.
     *
    ...
    public synchronized void activateOptions() {
        // Start the underlying generic appender with a handler object that pumps
        // statistics to the downstream appenders
        ...
        // Also, we add a shutdown hook that will attempt to flush any pending log
        // events in the queue.
        if (shutdownHook == null) {
            try {
                ...
                /* likely a security exception, nothing we can do */ }
        }
    }

    // --- attributes ---
    /**
     * Returns the number of StopWatch messages that have been discarded due to the
     * queue being full.
     *
    ...
    public void close() {
        ...
        // close the downstream appenders synchronized (downstreamAppenders) { //first
        // FLUSH any flushable downstream appenders (fix for PERFFORJ-22). Note we CAN
        // NOT just flush and
        // close in one loop because this breaks in the case of a "diamond" relationship
        // between appenders, where,
        // say, this appender has 2 attached GraphingStatisticsAppenders that each write
        // to a SINGLE attached
        // FileAppender.
        @SuppressWarnings({ "rawtypes", "unchecked" })
        ...
        while (enumer != null && enumer.hasMoreElements()) {
            ...
            if (appender instanceof Flushable) {
                try {
                    ...
                    /* Just eat the exception, we're closing down */ }
            }
        } // THEN close them
        @SuppressWarnings({ "rawtypes", "unchecked" })
        Enumeration<Appender> enumer2 = downstreamAppenders.getAllAppenders();
        while (enumer2 != null && enumer2.hasMoreElements()) {
            enumer2.nextElement().close();
        }
    ...
    // --- helper methods ---
    /**
     * Creates the new GenericAsyncCoalescingStatisticsAppender that this instance
     * will wrap.
     *
